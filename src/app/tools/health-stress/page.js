...
export default function HealthStressToolPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('vinca_tool_health_formData');
    if (stored) {
      try {
        setFormData((prev) => ({ ...prev, ...JSON.parse(stored) }));
      } catch (error) {
        console.warn('Unable to load stored health stress inputs', error);
      }
    }
  }, []);
  ...
  return (
    <div className="py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-10">
        <ToolWelcomeHeader toolName="Health Stress Test" />
        ...
        <ToolStepperInputs
          ...
          ctaLabel="Analyze your Health Readiness"
        />
        ...
        <div ref={summaryRef} className="space-y-4">
          ...
          {hasSubmitted && (
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6 text-center space-y-3">
              <p className="text-sm text-slate-700">Your Health Readiness preview is ready. Continue to unlock the detailed dashboard.</p>
              <button
          ...
          {!hasSubmitted && (
            <div className="bg-white border border-dashed border-slate-200 rounded-xl p-6 text-center text-slate-600">
              Complete all 3 steps and hit Analyze your Health Readiness to view your status banner preview.
            </div>
          )}
        </div>
      </div>
